...
build_libzimg() {
  do_git_checkout https://github.com/sekrit-twc/zimg.git zimg_git release-3.0.5
  cd zimg_git
    generic_configure_make_install
  cd ..
}
...
#    config_options+=" --enable-liblensfun"
    ...
    config_options+=" --enable-muxer=mp4,webm,matroska,null,gif,image2,image2pipe,apng"
    config_options+=" --enable-demuxer=png_pipe,matroska,gif,gif-pipe,image2,image2pipe,mov,svg_pipe"
    config_options+=" --enable-encoder=gif,png,libvpx,libvpx-vp9,apng "
...
#fi
ffmpeg_git_checkout_version=n7.1
build_ismindex=n
enable_gpl=y
